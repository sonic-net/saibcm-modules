 ...
 * Device family types.
 */
typedef enum bcmpkt_dev_info_family_types_e {
    BCMPKT_DEV_INFO_FAMILY_T_NONE, /*! Type not specified. */
    BCMPKT_DEV_INFO_FAMILY_T_XFS,  /*! XFS type header. */
    BCMPKT_DEV_INFO_FAMILY_T_XGS,  /*! XGS type header. */
    BCMPKT_DEV_INFO_FAMILY_T_XFS_XGS,  /*! XFS+XGS type header. */
    BCMPKT_DEV_INFO_FAMILY_T_DNX,  /*! DNX type header. */
    BCMPKT_DEV_INFO_FAMILY_T_DNXF, /*! DNX type header. */
    BCMPKT_DEV_INFO_FAMILY_T_COUNT /*! Must be end. */
} bcmpkt_dev_info_family_types_t;
...
/*!
 * \name DMA device configuration flags.
 * \anchor BCMPKT_DMA_DEV_XXX
 */
/*! \{ */
/*!
 * Set this flag to launch a separate Rx thread for each Rx DMA channel.
 * On multi-core CPU systems, this allows all Rx DMA channels to be processed in
 * parallel. Setting this flag on a single-core CPU system will most likely have
 * a negative impact on performance.
 */
#define BCMPKT_DMA_DEV_RX_POLL_SQ       (1 << 0)
/*! \} */
...
typedef struct bcmpkt_dev_init_s {
    ...
    /*! Configuration flags, refer to \ref BCMPKT_DMA_DEV_XXX. */
    uint32_t flags;
...
/*!
 * \brief Packet module suspend function.
 *
 * This function is to be called to suspend packet module.
 *
 * - Suspend all packet DMA channels.
 * - SW resources of packet module are preserved.
 *
 * \param [in] unit Switch unit number.
 * \param [in] graceful Indicating graceful or abrupt suspend.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_UNIT Invalid unit.
 * \retval SHR_E_PARAM Check parameters failed.
 * \retval SHR_E_FAIL Access driver failed.
 */
typedef int (*bcmpkt_dev_suspend_f) (int unit, bool graceful);

/*!
 * \brief Packet module resume function.
 *
 * This function is to be called to resume packet module.
 *
 * - Resume all packet DMA channels.
 *
 * \param [in] unit Switch unit number.
 *
 * \retval SHR_E_NONE No errors.
 ...
 * \retval SHR_E_FAIL Access driver failed.
 */
typedef int (*bcmpkt_dev_resume_f) (int unit);
...
/*!
 * \brief Per device info structure.
 */
typedef struct bcmpkt_dev_info_s {

    /*! Family type of the device. */
    bcmpkt_dev_info_family_types_t family_type;
 ...
 */
typedef int(*bcmpkt_dev_info_get_f)(int unit,
                                    bcmpkt_dev_info_t *info);
...
typedef struct bcmpkt_dev_s {
    ...
    /*! Packet device suspend function. */
    bcmpkt_dev_suspend_f suspend;

    /*! Packet device resume function. */
    bcmpkt_dev_resume_f resume;
    ...
    bcmpkt_dev_info_get_f dev_info_get;
 ...
 * This function is to be called to suspend packet module.
 *
 * - Suspend by stoping all packet DMA channels.
 * - Active netifs and SOCKETs are changing to idle.
 * - Allocated memories are all preserved.
 *
 * \param [in] unit Switch unit number.
 * \param [in] graceful Indicating graceful or abrupt suspend.
 *
 ...
 */
extern int
bcmpkt_dev_suspend(int unit, bool graceful);
