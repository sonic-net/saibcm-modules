...
/*!
 * \brief Memory pool information types.
 *
 * The types of information that can be extracted for debugging,
 * testing and statistical purposes.
 */
typedef enum bcmdrd_hal_dma_info_e {

    /*!
     * Total amount of DMA memory available (in bytes), i.e. the size
     * of any reserved DMA memory plus the size of the pool of DMA
     * memory used for dynamic allocations.
     */
    BCMDRD_HAL_DMA_INFO_MEM_TOTAL,

    /*!
     * Size of reserved DMA memory, e.g. for HA purposes (in bytes).
     */
    BCMDRD_HAL_DMA_INFO_MEM_RESERVED,

    /*!
     * Largest amount of DMA memory in use since pool creation. This
     * value includes all control overhead and allocation "holes",
     * such that it can be use to estimate the required amount of DMA
     * memory for a given system configuration.
     */
    BCMDRD_HAL_DMA_INFO_MEM_MAX,

    /*!
     * Amount of DMA memory currently allocated (in bytes).
     */
    BCMDRD_HAL_DMA_INFO_USAGE,

    /*!
     * DMA blocks are allocated from chunks of this size.
     */
    BCMDRD_HAL_DMA_INFO_CHUNK_SIZE

} bcmdrd_hal_dma_info_t;
...
/*!
 * \brief Get DMA memory information.
 *
 * Optional information that can be requested from the
 * application-provided DMA memory allocator. Please refer to \ref
 * bcmdrd_hal_dma_info_t for a description of the types of information
 ...
 *
 * \retval 0 No errors.
 * \retval -1 Unsupported information type.
 */
typedef int (*bcmdrd_hal_dma_info_get_f)(void *devh,
                                         bcmdrd_hal_dma_info_t type,
                                         uint32_t *data);
...
typedef struct bcmdrd_hal_dma_s {
    ...
    /*! Get DMA memory information from system implementation. */
    bcmdrd_hal_dma_info_get_f dma_info_get;
 ...
 *
 * The \c irq_num parameter is used to associate the interrupt handler
 * with a specific interrupt line, if the device supports multiple
 * interrupt lines (e.g. MSI vectors) per device.
 *
 * \param [in] devh Application-provided device handle.
 * \param [in] irq_num Interrupt line number (starting at zero).
 ...
 * function.
 *
 * The \c irq_num parameter is used to associate the access with a
 * specific interrupt line, if the device supports multiple interrupt
 * lines (e.g. MSI vectors) per device.
 *
 * \param [in] devh Application-provided device handle.
 * \param [in] irq_num Interrupt line number (starting at zero).
 ...
 * \anchor BCMDRD_HAL_IRQ_REG_F_xxx
 */

/*! \{ */

/*! Interrupt mask register is of type "write 1 to clear". */
#define BCMDRD_HAL_IRQ_REG_F_W1TC       (1 << 0)

/*!
 * Indicates that the interrupt status register is a bitwise AND of
 * mask and raw status, i.e. there is no need to read the interrupt
 * mask register to determine which interrupts are active.
 */
#define BCMDRD_HAL_IRQ_REG_F_MASKED     (1 << 1)

/*!
 * Indicates that the \c kmask value is valid. This is mainly to
 * distinguish a \c kmask value of zero from the \c kmask value being
 * uninitialized, as this matters during a warm boot, where we do not
 * want to disturb e.g. an active Linux KNET driver.
 */
#define BCMDRD_HAL_IRQ_REG_F_KMASK      (1 << 2)
 ...
 *
 * An optional kernel mask (\c kmask) can be used to indicate that
 * certain interrupts are handled by a secondary interrupt handler in
 * the system kernel (for example a Linux KNET driver).
 */
typedef struct bcmdrd_hal_irq_reg_s {
    ...
    /*! Mask of interrupts handled by primary (SDK) ISR. */
    uint32_t umask;

    /*! Mask of interrupts handled by secondary (system) ISR. */
    uint32_t kmask;

    /*! Flags for special handling (\ref BCMDRD_HAL_IRQ_REG_F_xxx). */
    uint32_t flags;
 ...
 * \anchor BCMDRD_HAL_IRQ_ACK_F_xxx
 */

/*! \{ */

/*! ACK registers resides in PCI bridge I/O window. */
#define BCMDRD_HAL_IRQ_ACK_F_PAXB       (1 << 0)
 ...
 *
 */
typedef struct bcmdrd_hal_irq_ack_reg_s {
...
typedef struct bcmdrd_hal_intr_info_s {
    ...
    bcmdrd_hal_irq_ack_reg_t irq_ack;
 ...
 * interrupt context.
 *
 * The \c irq_num parameter is used to associate the interrupt
 * configuration with a specific interrupt line, if the device
 * supports multiple interrupt lines (e.g. MSI vectors) per device.
 *
 * \param [in] devh Application-provided device handle.
 * \param [in] irq_num Interrupt line number (starting at zero).
 ...
 *
 * This function is used by the interrupt driver to allocate multiple
 * interrupt lines for a single device (if supported by the system).
 *
 * Some systems may benefit from having multiple per-device interrupt
 * lines, and the interrupt driver can use the \c irq_max parameter
 * for this purpose by using a value greater than one.
 *
 * This function is optional and if unsupported, a single interrupt
 * line is assumed.
 *
 * \param [in] devh Application-provided device handle.
 * \param [in] irq_max Desired number of per-device interrupt lines.
 * \param [out] num_irq Actual number of per-device interrupt lines.
 *
 * \retval 0 No errors
 * \retval <0 Something went wrong.
 */
typedef int (*bcmdrd_hal_intr_init_f)(void *devh, int irq_max, int *num_irq);
...
typedef struct bcmdrd_hal_intr_s {
    ...
    bcmdrd_hal_intr_init_f intr_init;
 ...
 * \param [in] unit Unit number.
 * \param [in] addr Register offset relative to register base.
 * \param [out] val64 Data read from register.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_FAIL I/O read returned an error.
 */
extern int
bcmdrd_hal_iproc_read64(int unit, uint32_t addr, uint64_t *val64);
 ...
 * \param [in] unit Unit number.
 * \param [in] addr Register offset relative to register base.
 * \param [in] val64 Data to write to register.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_FAIL I/O read returned an error.
 */
extern int
bcmdrd_hal_iproc_write64(int unit, uint32_t addr, uint64_t val64);
 ...
 *
 * Assign delay timer value to clear specific MSI interrupt line.
 * This is only used for MSI auto-clear mode.
 *
 * \param [in] unit Unit number.
 * \param [in] intr_num MSI interrupt line.
 ...
 * \brief Get DMA memory information.
 *
 * Optional information provided by the application-provided DMA
 * memory allocator. Please refer to \ref bcmdrd_hal_dma_info_t for a
 * description of the types of information that may be requested.
 *
 * \param [in] unit Unit number.
 * \param [in] type Information type.
 * \param [out] data Information data.
 ...
 */
extern int
bcmdrd_hal_dma_info_get(int unit, bcmdrd_hal_dma_info_t type, uint32_t *data);

/*!
 * \brief Initialize system interrupts.
 *
 * This function is used by the user-mode interrupt driver to allocate
 * multiple interrupt lines if supported by the system.
 *
 * Some systems may benefit from having multiple per-device interrupt
 * lines, and the user mode driver can use the \c irq_max parameter
 * for this purpose by using a value >1.
 *
 * \param [in] unit Unit number.
 * \param [in] irq_max Desired number of per-device interrupt lines.
 * \param [out] num_irq Actual number of per-device interrupt lines.
 ...
 */
extern int
bcmdrd_hal_intr_init(int unit, int irq_max, int *num_irq);

/*!
 * \brief Register interrupt handler with the system.
 *
 * The \c irq_num parameter is used to associate the interrupt
 * handler with a specific interrupt line, if the device supports
 * multiple interrupt lines (e.g. MSI vectors) per device.
 *
 * \param [in] unit Unit number.
 * \param [in] irq_num Interrupt line number (starting at zero).
 * \param [in] isr_func Interrupt handler function.
