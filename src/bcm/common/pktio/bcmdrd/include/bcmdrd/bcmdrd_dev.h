...
/*!
 * \brief Read 64-bit iProc register.
 *
 * The register address can be specified anywhere in the 32-bit AXI
 * address space.
 *
 * If supported by the host CPU, the access will be performed
 * atomically.
 *
 * \param [in] _unit Unit number.
 * \param [in] _addr Absolute register offset in AXI address space.
 * \param [out] _val64 Data read from register.
 */
#define BCMDRD_IPROC_READ64(_unit, _addr, _val64) \
    bcmdrd_hal_iproc_read64(_unit, _addr, _val64)

/*!
 * \brief Write 64-bit iProc register.
 *
 * The register address can be specified anywhere in the 32-bit AXI
 * address space.
 *
 ...
 * \param [in] _unit Unit number.
 * \param [in] _addr Absolute register offset in AXI address space.
 * \param [in] _val64 Data to write to register.
 */
#define BCMDRD_IPROC_WRITE64(_unit, _addr, _val64) \
    bcmdrd_hal_iproc_write64(_unit, _addr, _val64)
...
/*!
 * \brief Set chip profile.
 *
 * A chip profile contains information about how a given SKU variant
 * differs from the base device.
 *
 * Note that only a pointer to the chip profile will be stored in the
 * DRD device structure.
 *
 * If a NULL pointer is passed in as the chip profile pointer, then
 * the default chip profile for this SKU device will be loaded.
 *
 * \param [in] unit Unit number.
 * \param [in] profile Pointer to chip profile.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_UNIT Invalid unit number.
 */
extern int
bcmdrd_dev_chip_profile_set(int unit, bcmdrd_chip_profile_t *profile);

/*!
 * \brief Get a pointer to the current chip profile.
 *
 * \param [in] unit Unit number.
 *
 * \return Pointer to chip profile, or NULL if none found.
 */
extern bcmdrd_chip_profile_t *
bcmdrd_dev_chip_profile_get(int unit);
 ...
 *
 * \param [in] unit Unit number.
 * \param [in] pm Bitmap of valid pipes.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_UNIT Invalid unit number.
 */
extern int
bcmdrd_dev_valid_pipemap_set(int unit, bcmdrd_pipemap_t *pm);
 ...
 *
 * \param [in] unit Unit number.
 * \param [out] pm Bitmap of valid pipes.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_UNIT Invalid unit number.
 ...
 */
extern int
bcmdrd_dev_valid_pipemap_get(int unit, bcmdrd_pipemap_t *pm);
 ...
 * Override the default set of valid pipes for this unit.
 *
 * Note that this API only supports 32 pipes or fewer.
 *
 * See also \ref bcmdrd_dev_valid_pipemap_set.
 *
 * \param [in] unit Unit number.
 ...
 * Note that this API only supports 32 pipes or fewer.
 *
 * See also \ref bcmdrd_dev_valid_pipemap_get.
 *
 * \param [in] unit Unit number.
...
/*!
 * \brief Device-specific pipe number check.
 *
 * Check if pipe number is valid for a given block type.
 *
 * \param [in] unit Unit number.
 * \param [in] blktype Device-specific block type.
 * \param [in] pipe_no Pipe number to check.
 *
 * \retval true Pipe number is valid.
 * \retval false Pipe number is not valid.
 */
typedef bool (*bcmdrd_dev_block_pipe_valid_f)(int unit, int blktype,
                                              int pipe_no);

/*!
 * \brief Set device-specific function for checking of pipe number.
 *
 * This function installs a callback for the \ref
 * bcmdrd_dev_block_pipe_valid API. The callback will override the
 * default behavior of this API.
 *
 * \param [in] unit Unit number.
 * \param [in] func Device-specific pipe number check function.
 *
 * \retval SHR_E_NONE Function successfully installed.
 * \retval SHR_E_UNIT Invalid unit number.
 */
extern int
bcmdrd_dev_block_pipe_valid_func_set(int unit,
                                     bcmdrd_dev_block_pipe_valid_f func);

/*!
 * \brief Check if pipe number is valid for a given block type.
 *
 * \param [in] unit Unit number.
 * \param [in] blktype Device-specific block type.
 * \param [in] pipe_no Pipe number to check.
 ...
 */
extern bool
bcmdrd_dev_block_pipe_valid(int unit, int blktype, int pipe_no);
 ...
 * \param [in] unit Unit number.
 * \param [in] blktype Device-specific block type.
 * \param [out] pm Bitmap of valid pipes for the specified block.
 *
 * \retval SHR_E_NONE No errors.
 * \retval SHR_E_UNIT Invalid unit number.
 * \retval SHR_E_PARAM Invalid parameter.
 */
extern int
bcmdrd_dev_valid_block_pipes_get(int unit, int blktype, bcmdrd_pipemap_t *pm);
 ...
 *
 * This setting may have different meanings on different devices all
 * depending on how the \c opmode is interpreted by the target device.
 *
 * \param [in] unit Unit number.
 * \param [in] opmode Device operation mode.
 *
 * \return Nothing.
 */
extern void
bcmdrd_dev_opmode_set(int unit, uint32_t opmode);

/*!
 * \brief Get device operation mode.
 *
 * See \ref bcmdrd_dev_opmode_set for details.
 *
 * \param [in] unit Unit number.
 *
 * \return Device operation mode.
 */
extern uint32_t
bcmdrd_dev_opmode_get(int unit);

/*!
 * \brief Set device bus initialization state.
 *
 * During warmboot it may interfere with ongoing device activities
 * (e.g. in a Linux kernel module) if the device bus is reinitialized.
 * This API allows system-level code to indicate that the device bus
 * has already been initialized.
 *
 * \param [in] unit Unit number.
 * \param [in] bus_initialized Whether device bus has been initialized.
 *
 * \return Nothing.
 */
extern void
bcmdrd_dev_bus_initialized_set(int unit, bool bus_initialized);

/*!
 * \brief Check if device bus has been initialized.
 *
 * Low-level device bus drivers (e.g. PCI bridge drivers) should
 * consult this API to avoid reinitializing the device bus. This is
 * especially important during warmboot where the device may be active
 * even before the application is loaded.
 *
 * \param [in] unit Unit number.
 *
 * \return Whether device bus has been initialized.
 */
extern bool
bcmdrd_dev_bus_initialized_get(int unit);

/*!
 * \brief Get license data.
 *
 * Get hardware license data for download into the device.
 *
 * Currently only a single license is supported.
 *
 * \param [in] unit Unit number.
 * \param [out] size Size of license data.
 *
 * \return License data or NULL if unavailable.
 */
extern uint8_t *
bcmdrd_dev_license_get(int unit, size_t *size);

/*!
 * \brief Remove all license data.
 *
 * This operation is also performed implicitly when calling \ref
 * bcmdrd_dev_destroy.
 *
 * \param [in] unit
 *
 ...
 */
extern int
bcmdrd_dev_license_cleanup(int unit);
...
/*!
 * \brief Get block pipe number associated with device pipe.
 *
 * \param [in] unit Unit number.
 * \param [in] blktype Device-specific block type.
 * \param [in] dev_pipe_no Device pipe number.
 *
 * \return Block pipe number or -1 on error.
 */
extern int
bcmdrd_dev_block_pipe(int unit, int blktype, int dev_pipe_no);
 ...
 */
extern int
bcmdrd_dev_num_block_inst(int unit, int blktype);
 ...
 *
 * This function works like \ref bcmdrd_chip_block_types_match, but
 * also provides an indicator of whether the block has any valid
 * ports. The function uses a cached value for this indicator, since
 * it is relatively expensive to calculate it each time the function
 * is called.
 *
 * Please refer to \ref bcmdrd_chip_block_types_match for a detailed
 * description of the \c info and \c pblk parameters.
 *
 * \param [in] unit Unit number.
 ...
 * \param [out] valid_ports Whether this block has any valid ports.
 *
 * \retval 0 No errors
 */
extern const bcmdrd_block_t *
bcmdrd_dev_block_types_match(int unit, uint32_t info,
...
/*!
 * \brief Device-specific function for registers/memories validity.
 *
 * If there are exceptions of registers/memories validaty, this function can
 * be implemented as device-specific to reflect the physical table validity
 * exceptions at system initialization stage.
 *
 * \param [in] unit Unit number.
 * \param [in] sid Symbol ID to check.
 *
 * \retval true SID is valid for the current device configuration.
 * \retval false SID is not valid for the current device configuration.
 */
typedef bool (*bcmdrd_dev_pt_valid_f)(int unit, bcmdrd_sid_t sid);

/*!
 * \brief Set device-specific function of physical table validity.
 *
 * Set device-specific physical table validity function to the device
 * resource database. Set \c func to NULL to clean up the database.
 *
 * \param [in] unit Unit number.
 * \param [in] func Device-specific physical table validity function.
 *
 * \retval SHR_E_NONE Function successfully installed.
 * \retval SHR_E_UNIT Invalid unit number.
 */
extern int
bcmdrd_dev_pt_valid_func_set(int unit, bcmdrd_dev_pt_valid_f func);

/*!
 * \brief Check exceptions for registers/memories validity.
 *
 * This function can be used to check if there is any run-time exception of
 * physical table that can not be accessed.
 *
 * \param [in] unit Unit number.
 * \param [in] sid Symbol ID to check.
 *
 ...
 */
extern bool
bcmdrd_dev_pt_valid(int unit, bcmdrd_sid_t sid);

/*!
 * \brief Check if physical table index range is contiguous.
 *
 * Please refer to \ref bcmdrd_dev_pt_contiguous for more information.
 *
 * \param [in] unit Unit number.
 * \param [in] sid Symbol ID to check.
 *
 * \retval true Table index range is contiguous (no holes).
 * \retval false Table has holes in the index range.
 */
typedef bool (*bcmdrd_dev_pt_contiguous_f)(int unit, bcmdrd_sid_t sid);

/*!
 * \brief Set device-specific check for physical table index range.
 *
 * Set device-specific function that reports whether the indexes of a
 * physical table are contiguous. Set \c func to NULL to use the
 * default checker.
 *
 * \param [in] unit Unit number.
 * \param [in] func Device-specific function for index table map.
 *
 * \retval SHR_E_NONE Function successfully installed.
 * \retval SHR_E_UNIT Invalid unit number.
 */
extern int
bcmdrd_dev_pt_contiguous_func_set(int unit, bcmdrd_dev_pt_contiguous_f func);

/*!
 * \brief Check if physical table index range is contiguous.
 *
 * Table DMA operations typically require that all indexes in a range
 * are valid and accessible. This function checks if this is true for
 * a given table, which in turn allows the DMA driver to check each
 * table index for validity only for tables with holes.
 *
 * \param [in] unit Unit number.
 * \param [in] sid Symbol ID to check.
 *
 ...
 */
extern bool
bcmdrd_dev_pt_contiguous(int unit, bcmdrd_sid_t sid);

/*!
 * \brief Remap two-dimensional table instance to pipe and block instance.
 *
 * For symbol ID with two-dimensional table instance composed of pipe and
 * block, this function can be used to remap the table instance to pipe
 * instance and block instance.
 *
 * \param [in] unit Unit number.
 * \param [in] sid Symbol ID to check.
 * \param [in] tbl_inst Table instance to check.
 * \param [out] pipe remapped pipe instance.
 * \param [out] blkinst remapped block instance.
 *
 * \retval SHR_E_NONE The \c pipe and \c blkinst are successfully remapped.
 * \retval SHR_E_UNIT The Unit number \c unit is invalid.
 * \retval SHR_E_PARAM The returned parameter \c pipe or \c blkinst is NULL.
 * \retval SHR_E_UNAVAIL No remap function available for the \c unit or \c sid.
 */
typedef int (*bcmdrd_dev_pt_inst_remap_f)(int unit, bcmdrd_sid_t sid,
                                          int tbl_inst,
                                          int *pipe, int *blkinst);

/*!
 * \brief Set device-specific function to remap two-dimensional table instance.
 *
 * Set device-specific table instance remap function to the device
 * resource database. Set \c func to NULL to clean up the database.
 *
 * \param [in] unit Unit number.
 * \param [in] func Device-specific table instance remap function.
 *
 * \retval SHR_E_NONE Function successfully installed.
 * \retval SHR_E_UNIT Invalid unit number.
 */
extern int
bcmdrd_dev_pt_inst_remap_func_set(int unit, bcmdrd_dev_pt_inst_remap_f func);
 ...
 */
extern int
bcmdrd_dev_pt_inst_remap(int unit, bcmdrd_sid_t sid, int tbl_inst,
                         int *pipe, int *blkinst);
