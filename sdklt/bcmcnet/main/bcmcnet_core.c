...
bcmcnet_pdma_dev_start(struct pdma_dev *dev)
{
    ...
    for (qi = 0; qi < ctrl->nb_rxq; qi++) {
        ...
        at_clear_bit(qi, &ctrl->bm_rxq_isr, ctrl->lock);
    ...
    for (qi = 0; qi < ctrl->nb_txq; qi++) {
        ...
        at_clear_bit(qi, &ctrl->bm_txq_isr, ctrl->lock);
...
bcmcnet_pdma_dev_stop(struct pdma_dev *dev)
{
    ...
    for (qi = 0; qi < ctrl->nb_rxq; qi++) {
        ...
        retry = 0;
        while (at_test_set_bit(qi, &ctrl->bm_rxq_isr, ctrl->lock)) {
            sal_usleep(1000);
            if (++retry > 1000) {
                CNET_ERROR(dev->unit, "Timeout to stop Rx queue %d\n", qi);
    ...
    for (qi = 0; qi < ctrl->nb_txq; qi++) {
        ...
        retry = 0;
        while (at_test_set_bit(qi, &ctrl->bm_txq_isr, ctrl->lock)) {
            sal_usleep(1000);
            if (++retry > 1000) {
                CNET_ERROR(dev->unit, "Timeout to stop Tx queue %d\n", qi);
                return SHR_E_TIMEOUT;
            }
        }
