...
/*! \cond */
static int napi_sq = 0;
MODULE_PARAM(napi_sq, int, 0);
MODULE_PARM_DESC(napi_sq,
"Use single queue NAPI (default 0 to use one NAPI for all queues)");
/*! \endcond */
...
ngknet_ndev_init(ngknet_netif_t *netif, struct net_device **nd)
{
    ...
    /* Update name space to support IOCTL from Linux containers */
...
ngknet_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case NGKNET_DEV_INIT:
        ...
        if (napi_sq || dev_cfg->flags & NGKNET_DEV_POLL_SQ) {
            pdev->flags &= ~PDMA_GROUP_INTR;
        }
