...
#define PSAMPLE_EGR_RATE_DFLT 0
#define PSAMPLE_EGR_SIZE_DFLT 0
...
static void
psample_config_get(bcmgenl_netif_t *psample_netif, bool egress_netif,
                   int rx_sample_type, int *rate, int *size)
{
    int match_type = egress_netif ? SAMPLE_TYPE_EGRESS : SAMPLE_TYPE_INGRESS;
    int mismatch_type = egress_netif ? SAMPLE_TYPE_INGRESS : SAMPLE_TYPE_EGRESS;

    /*
     *Use egr_sample_size to disable/enable separate configuration sets support.
     */
    if (psample_netif->egr_sample_size == 0) {
        /*
         * Use single configuration set.
         * Sampling rate and size are from ingress network interface
         * regardless of the sampling type.
         */
        if (!egress_netif) {
            *rate = psample_netif->sample_rate;
            *size = psample_netif->sample_size;
        }
        return;
    }

    /*
     * Use separate configuration sets for ingress and egress sampling.
     */
    if (rx_sample_type == mismatch_type) {
        return;
    }
    if (rx_sample_type == match_type) {
        if (egress_netif) {
            *rate = psample_netif->egr_sample_rate;
            *size = psample_netif->egr_sample_size;
        } else {
            *rate = psample_netif->sample_rate;
            *size = psample_netif->sample_size;
        }
    } else if (*rate == PSAMPLE_RATE_DFLT && *size == PSAMPLE_SIZE_DFLT) {
        if (egress_netif) {
            if (psample_netif->egr_sample_rate != PSAMPLE_EGR_RATE_DFLT ||
                psample_netif->egr_sample_size != PSAMPLE_EGR_SIZE_DFLT) {
                *rate = psample_netif->egr_sample_rate;
                *size = psample_netif->egr_sample_size;
            }
        } else {
            if (psample_netif->sample_rate != PSAMPLE_RATE_DFLT ||
...
bcmgenl_psample_meta_get(struct sk_buff *skb, bcmgenl_pkt_t *bcmgenl_pkt, psample_meta_t *sflow_meta)
{
    ...
    if ((psample_netif = psample_netif_lookup_by_port(srcport))) {
        ...
        psample_config_get(psample_netif, false, bcmgenl_pkt->meta.sample_type,
    ...
    if ((dstport != 0) &&
        ...
        psample_config_get(psample_netif, true, bcmgenl_pkt->meta.sample_type,
                           &sample_rate, &sample_size);
...
bcmgenl_psample_netif_create_cb(ngknet_dev_info_t *dinfo, ngknet_netif_t *netif)
{
    ...
    new_netif->egr_sample_rate = PSAMPLE_EGR_RATE_DFLT;
    new_netif->egr_sample_size = PSAMPLE_EGR_SIZE_DFLT;
    ...
    size_t count, loff_t *loff)
{
    ...
    *ptr++ = 0;
    ...
    if (!found) {
        printk("Warning: Failed setting psample size on unknown network "
               "interface: '%s'\n", sample_str);
    }
    return count;
}
...
/*
 * psample egr_rate Proc Read Entry
 */
static int
bcmgenl_psample_proc_egr_rate_show(struct seq_file *m, void *v)
{
    struct list_head *list;
    bcmgenl_netif_t *psample_netif;
    unsigned long flags;
    ...
    list_for_each(list, &g_bcmgenl_psample_info.netif_list) {
        psample_netif = (bcmgenl_netif_t*)list;
        seq_printf(m, "  %-14s %d\n",
                   psample_netif->dev->name, psample_netif->egr_sample_rate);
    }
...
static int
bcmgenl_psample_proc_egr_rate_open(struct inode * inode, struct file * file)
{
    return single_open(file, bcmgenl_psample_proc_egr_rate_show, NULL);
}

/*
 * psample egr_rate Proc Write Entry
 *
 *   Syntax:
 *   <netif>=<pkt sample rate>
 *
 ...
 */
static ssize_t
bcmgenl_psample_proc_egr_rate_write(
    struct file *file, const char *buf,
    size_t count, loff_t *loff)
{
    bool found;
    ...
    if ((ptr = strchr(sample_str, '=')) == NULL &&
        (ptr = strchr(sample_str, ':')) == NULL) {
        printk("Error: Pkt sample egr_rate syntax not recognized: '%s'\n",
               sample_str);
        return count;
    }
    *ptr++ = 0;
    ...
    list_for_each(list, &g_bcmgenl_psample_info.netif_list) {
        psample_netif = (bcmgenl_netif_t*)list;
        if (strcmp(psample_netif->dev->name, sample_str) == 0) {
            psample_netif->egr_sample_rate = simple_strtol(ptr, NULL, 10);
            found = true;
            break;
        }
    }
    ...
    if (!found) {
        printk("Warning: Failed setting psample egr_rate on unknown network "
               "interface: '%s'\n", sample_str);
    }
    return count;
}

struct proc_ops bcmgenl_psample_proc_egr_rate_file_ops = {
    PROC_OWNER(THIS_MODULE)
    .proc_open =       bcmgenl_psample_proc_egr_rate_open,
    .proc_read =       seq_read,
    .proc_lseek =      seq_lseek,
    .proc_write =      bcmgenl_psample_proc_egr_rate_write,
    .proc_release =    single_release,
};

/*
 * psample egr_size Proc Read Entry
 */
static int
bcmgenl_psample_proc_egr_size_show(struct seq_file *m, void *v)
{
    struct list_head *list;
    bcmgenl_netif_t *psample_netif;
    unsigned long flags;
    ...
    list_for_each(list, &g_bcmgenl_psample_info.netif_list) {
        psample_netif = (bcmgenl_netif_t*)list;
        seq_printf(m, "  %-14s %d\n",
                   psample_netif->dev->name, psample_netif->egr_sample_size);
    }
...
static int
bcmgenl_psample_proc_egr_size_open(struct inode * inode, struct file * file)
{
    return single_open(file, bcmgenl_psample_proc_egr_size_show, NULL);
}

/*
 * psample egr_size Proc Write Entry
 *
 *   Syntax:
 *   <netif>=<pkt sample size in bytes>
 *
 ...
 */
static ssize_t
bcmgenl_psample_proc_egr_size_write(
    struct file *file, const char *buf,
    size_t count, loff_t *loff)
{
    bool found;
    ...
    if ((ptr = strchr(sample_str, '=')) == NULL &&
        (ptr = strchr(sample_str, ':')) == NULL) {
        printk("Error: Pkt sample egr_size syntax not recognized: '%s'\n",
               sample_str);
        return count;
    }
    *ptr++ = 0;
    ...
    list_for_each(list, &g_bcmgenl_psample_info.netif_list) {
        bcmgenl_netif = (bcmgenl_netif_t*)list;
        if (strcmp(bcmgenl_netif->dev->name, sample_str) == 0) {
            bcmgenl_netif->egr_sample_size = simple_strtol(ptr, NULL, 10);
            found = true;
            break;
        }
    }
    ...
    if (!found) {
        printk("Warning: Failed setting psample egr_size on unknown network "
               "interface: '%s'\n", sample_str);
    }
    return count;
}

struct proc_ops bcmgenl_psample_proc_egr_size_file_ops = {
    PROC_OWNER(THIS_MODULE)
    .proc_open =       bcmgenl_psample_proc_egr_size_open,
    .proc_read =       seq_read,
    .proc_lseek =      seq_lseek,
    .proc_write =      bcmgenl_psample_proc_egr_size_write,
    .proc_release =    single_release,
};
...
psample_cb_proc_cleanup(void)
{
    ...
    remove_proc_entry("egr_rate", psample_proc_root);
    remove_proc_entry("egr_size", psample_proc_root);
...
psample_cb_proc_init(void)
{
    ...
    /* create procfs for setting egress sample rates */
    PROC_CREATE(entry, "egr_rate", 0666, psample_proc_root,
                &bcmgenl_psample_proc_egr_rate_file_ops);
    if (entry == NULL) {
        printk("%s: Unable to create procfs entry '/procfs/%s/egr_rate'\n",
               __func__, psample_procfs_path);
        return -1;
    }

    /* create procfs for setting egress sample size */
    PROC_CREATE(entry, "egr_size", 0666, psample_proc_root,
                &bcmgenl_psample_proc_egr_size_file_ops);
    if (entry == NULL) {
        printk("%s: Unable to create procfs entry '/procfs/%s/egr_size'\n",
               __func__, psample_procfs_path);
        return -1;
    }
