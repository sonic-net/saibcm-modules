...
ngknet_tx_frame_process(struct net_device *ndev, struct sk_buff **oskb)
{
    ...
    if (priv->netif.flags & NGKNET_NETIF_F_RCPU_ENCAP) {
        ...
        if (rch->flags & RCPU_FLAG_DATA_RAW) {
            pkh->attrs |= PDMA_TX_DATA_RAW;
        }
    } else {
    ...
    if (tag_len == 0 && (priv->netif.vlan & 0xfff) != 0 &&
        (!(pkh->attrs & PDMA_TX_HIGIG_PKT) ||
         priv->netif.flags & NGKNET_NETIF_F_ADD_TAG) && !(pkh->attrs & PDMA_TX_DATA_RAW)) {
...
/*!
 * \brief Lookup network device by netif name.
 *
 * \param [in] name Network device name.
 * \param [in] mac Network device MAC address.
 * \param [out] nd New registered network device.
    ...
    struct net_device **nd)
{
    ...
    /* get Ethernet device */
    ndev = netdev_get_by_name(current->nsproxy->net_ns, netif->name, NULL, GFP_ATOMIC);
    if (ndev == NULL) {
        *nd = NULL;
        DBG_WARN(("Error to get Ethernet device by name (%s).\n", netif->name));
        return;
    }

    /* Compare the device MAC address */
    ma = netif->macaddr;
    if (ndev->dev_addr[0] != ma[0] || ndev->dev_addr[1] != ma[1] ||
        ndev->dev_addr[2] != ma[2] || ndev->dev_addr[3] != ma[3] ||
        ndev->dev_addr[4] != ma[4] || ndev->dev_addr[5] != ma[5]) {
        *nd = NULL;
 ...
 */
static int
ngknet_sand_ndev_init(ngknet_netif_t *netif, struct net_device **nd)
{
    struct net_device *ndev = NULL;
    uint8_t *ma;
    int rv;
...
static int
ngknet_sand_netif_create(struct ngknet_dev *dev, ngknet_netif_t *netif)
{
    struct net_device *ndev = NULL;
    struct ngknet_private *priv = NULL;
    unsigned long flags;
    ...
    ngknet_sand_ndev_lookup(netif, &ndev);
    if (ndev == NULL) {
        if (netif->flags & NGKNET_NETIF_F_REPLACE) {
            return SHR_E_NOT_FOUND;
        }
        rv = ngknet_sand_ndev_init(netif, &ndev);
        if (SHR_FAILURE(rv)) {
            return rv;
        }
        priv = netdev_priv(ndev);
        ...
        if (netif->flags & NGKNET_NETIF_F_REPLACE) {
            /**
             * netdev_get_by_name() increases the device reference count,
             * netdev_put() is called to avoid a reference leak.
             */
            netdev_put(ndev, NULL);

            priv = netdev_priv(ndev);
            if (netif->flags & NGKNET_NETIF_F_WITH_ID) {
                if (netif->id != priv->netif.id) {
                    DBG_WARN(("ID %d is already in use\n", id));
                    return SHR_E_PARAM;
                }
            } else {
            ...
            if ((netif->flags & NGKNET_NETIF_F_USE_SHARED_NDEV) == 0) {
                netdev_put(ndev, NULL);
                return SHR_E_FAIL;
            }
            priv = netdev_priv(ndev);
    ...
    if (netif->flags & NGKNET_NETIF_F_WITH_ID) {
        if (id == 0 || id > NUM_VDEV_MAX) {
            ...
            if (netif->flags & NGKNET_NETIF_F_REPLACE) {
                if (!dev->vdev[id]) {
                    DBG_WARN(("ID %d is not in use. Replace operation failed.\n", id));
                    rv = SHR_E_BADID;
                }
            } else {
                if (dev->vdev[id]) {
    ...
    if (netif->flags & NGKNET_NETIF_F_REPLACE) {
        ...
        if ((netif->flags & NGKNET_NETIF_F_USE_SHARED_NDEV) == 0) {
            /*
             * If core_group changed, we should update the dev&netif here.
             */
            if (priv->bkn_dev != dev) {
                priv->bkn_dev = dev;
            }
        ...
        list_for_each(list, &dev->cbc->netif_create_cb_list) {
            netif_create_cb = list_entry(list, netif_cb_t, list);
            if (netif_create_cb->cb(&dev->dev_info, &priv->netif)) {
                DBG_WARN(("Network interface callback (replace) failed for '%s'\n",
                          ndev->name));
            }
        }
        DBG_VERB(("Replace virtual network device %s (%d).\n",
                  ndev->name, priv->netif.id));
        /* Replace return here. */
        return SHR_E_NONE;
    }
...
static int
ngknet_sand_netif_destroy(struct ngknet_dev *dev, int id)
{
    struct net_device *ndev = NULL;
    struct ngknet_private *priv = NULL;
    unsigned long flags;
...
ngknet_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case NGKNET_NETIF_CREATE:
        ...
        if (device_is_sand(ioc.unit)) {
            ioc.rc = ngknet_sand_netif_create(dev, netif);
        } else {
    ...
    case NGKNET_NETIF_DESTROY:
        ...
        if (device_is_sand(ioc.unit)) {
            ioc.rc = ngknet_sand_netif_destroy(dev, ioc.iarg[0]);
        } else {
    ...
    case NGKNET_PTP_DEV_CTRL:
        ...
        break;
    case NGKNET_MESSAGE:
        DBG_CMD(("NGKNET_MESSAGE\n"));
        if (ioc.op.data.len) {
            data = kmalloc(ioc.op.data.len, GFP_ATOMIC);
            if (data == NULL) {
                printk("Fatal error: no memory for message.\n");
                return -EFAULT;
            }
            if (copy_from_user(data, (void *)(unsigned long)ioc.op.data.buf,
                               ioc.op.data.len)) {
        ...
        ioc.rc = ngknet_adapter_msg(dev, ioc.unit, ioc.iarg[0], ioc.iarg[1], data, ioc.op.data.len);
        if (SHR_FAILURE((int)ioc.rc)) {
            if (data) {
                kfree(data);
            }
